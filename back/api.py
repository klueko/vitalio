...
from api_auth import requires_auth, requires_role, get_current_user_role, get_user_role
...
from services.user_service import (
    ...
    get_assigned_doctor_ids_for_patient, ensure_patient_access_or_403,
...
from services.invitation_service import (
    ...
    send_invitation_email, invite_emergency_contact_if_needed,
)
...
def get_my_profile():
    ...
    return jsonify({"profile": profile_data, "doctors": doctors}), 200
...
def create_doctor_invitation():
    ...
    if send_email and patient_email:
        ...
        def _send_async():
            try:
                send_invitation_email(patient_email, invite_token, web_invite_url, expires_at, doctor_display_name)
            except Exception as e:
...
def accept_caregiver_invitation():
    ...
    try:
        ...
        if get_user_role(caregiver_user_id_auth) not in ("caregiver", "aidant", "doctor", "admin"):
            get_identity_db().users.update_one({"user_id_auth": caregiver_user_id_auth}, {"$set": {"role": "caregiver"}})
...
def get_doctor_alerts():
    ...
    for alert in cursor:
        ...
        out["patient_id"] = patient_by_device.get(alert.get("device_id"))
        alerts.append(out)
...
def get_ml_forecast(patient_id: str):
    ...
    if len(measurements) < 3:
        return jsonify({"code": "insufficient_data", "message": f"Not enough measurements ({len(measurements)} < 3)", "patient_id": patient_id}), 400
    try:
...
def get_patient_ml_analysis(patient_id: str):
    ...
    measurements = query_patient_measurements_for_devices(device_ids=device_ids, days=days, limit=10000)
    if len(measurements) < 3:
        return jsonify({"code": "insufficient_data", "message": f"Not enough measurements ({len(measurements)} < 3)", "patient_id": patient_id}), 400
